...
1. **`label` is the machine id; `name` is the text shown to the user.** This
   holds for every API type (`DatabasesResponse`, `Facet`, `FacetField`, and
   `StatsResponse.label`, which is a database's `label`) and for the frontend's
   own `CatalogField` and `CatalogOperator`. In most codebases a "label" is what
   the user sees; here it never is.
2. **A `…Id` in state or in a query node holds a `label`.**
   `AppState.selectedDatabaseIds` holds `DatabasesResponse.label`s;
   `Condition.facetId`, `fieldId` and `operatorId` hold `Facet.label`,
   `CatalogField.label` (`"facetLabel.fieldLabel"`) and `CatalogOperator.label`
   (`"gt"`). Only a node's own `id` is generated by the frontend.
3. **A `…Response` type is one record from a response, not the whole body.**
   ...
   exception: it is the whole `/api/query` body.) The names match the backend's
   own types.

The same things have different names in the UI, on the wire (named by the real
backend, so they stay) and inside the mock server (which speaks the backend's
vocabulary):

| UI and `src/` | Wire (`src/api/types.ts`) | Mock server |
|---|---|---|
| event, `EventRecord` | `EventsResponse.entrysets`, `DatabasesResponse.totalEntrysets` | `Entryset`, `ENTRYSETS`, `data/entrysets.json` |
| facet, `Facet` | `GET /api/individuals` | `Individual`, `INDIVIDUALS`, `data/individual.json` |
| an event's facet values | `EventRecord.items` (keyed by facet label, then field label) | same |
...
| A stats line streams in | If its request is still current, it is appended to `stats.lines` → only the statistics panel repaints. When the stream ends, `status` becomes `"ok"`. |
...
- **Streaming is not staleness.** While `stats.status` is `"loading"`,
  `stats.lines` holds fewer lines than there are selected databases; every
  line it holds belongs to the query on screen, because the guard runs per
  line.
...
`AppState.databases` by its `label` and uses its `totalEntrysets` as the
denominator. `matchCount` is present only when `success` is true, so a failed
database can never be shown as "0 matched"; a failed line carries
`errorMessages` / `infoMessages` instead. The headline sums the successful
lines only and says how many databases it excludes.

### The field catalog (there is no schema endpoint)

The query builder's fields are derived **client-side** by `buildFieldCatalog`
from the facets already loaded: one `CatalogField` per (facet, field), with
label `"facetLabel.fieldLabel"`. The operators are the fixed `OPERATORS` list;
which ones a field offers depends only on its value type (`OPERATOR_PROFILE`).
...
- **Decision: no `GET /api/schema`.** The real backend has none, and adding
  ...
  normalised `valueType` field on `FacetField` instead.
...
backend needs (`kind`, `operator`, `children`, `fieldId`, `operatorId`,
`value`), the tree carries three the backend can ignore: every node's `id`, a
condition's `facetId` (implied by `fieldId`) and a group's display-only
`collapsed`.
...
(`AND`/`OR` over its `children`); the leaves are `Condition`s (`fieldId`,
`operatorId`, `value`, all `null` until chosen). The shape of `value` depends
on the operator's arity: `none` (no value), `one`, `two` (a `[from, to]`
pair) or `many` (a list).
...
(hover: `fieldTitle`). The backend sends `""` for missing text, so every text
goes through `text()` and blank parts are left out. The search box
(`matchDocs`: facet name, field label or name) hides what doesn't match and
...
dropdowns — **Facet**, **Field** (that facet's fields, by
...
headline (sum of successful lines, ratio, bar, "Excludes N databases that
failed"), then **By database** (one row per streamed line; failures in red),
...
- **Data:** 7 databases (`ALPHA`…`ETA`, sizes 12K to 5.6B) in `databases.ts`;
  ...
  event into a `Row` with `"facetLabel.fieldLabel"` keys plus a `__db` key;
  `evaluate.ts` matches the query against rows.
...
`eslint:recommended` + `typescript-eslint` recommended, plus the two import
...
**…add an operator?**
1. Add it to `OPERATORS` in `src/query/fieldCatalog.ts` (`label`, `name`,
...
4. Tests: `fieldCatalog`, `validate`, `summary`, `evaluate`. Tell the backend
   team — the operator's `label` is part of the wire format.
...
**…add an API endpoint?**
1. Its types in `src/api/types.ts`; a function in `src/api/client.ts` (use
   `request` / `requestNoContent`, which add the base path, timeout and
   `ApiError`).
