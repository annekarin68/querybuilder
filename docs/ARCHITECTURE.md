...
1. **`label` is the machine id; `name` is the text shown to the user.** This
   ...
   own `CatalogOperator`; a `CatalogField` has two, `facetLabel` and
   `fieldLabel`. In most codebases a "label" is what the user sees; here it
   never is.
2. **A `…Id` in state or in a query node holds a `label`.**
   ...
   `FacetField.label` (the field's own label, within that facet) and
   `CatalogOperator.label` (`"gt"`). A field is always named by the pair
   (facet label, field label), never by one joined string: a label may itself
   contain a `.`. Only a node's own `id` is generated by the frontend.
